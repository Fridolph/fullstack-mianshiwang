...
- 代码已经补好了 Nuxt UI 的全局 CSS
- 但浏览器有时还会短暂看到“像原生按钮 / 原生输入框”的旧样式
...
1. 先确认 `main.css` 是否已经导入 `tailwindcss` 和 `@nuxt/ui`
2. 再确认 `app.vue` 是否包了 `UApp`
3. 最后重启 dev server + 浏览器强刷一次

很多时候问题就停在第 3 步。

---

### 5. CORS 里只写 `*`，和 `credentials: include` 是冲突的

这次登录联调里又遇到了一个很典型的前后端协作问题：

现象：

- 前端已经把接口地址改成了 `http://localhost:6789/api`
- 请求也确实发到了正确后端
- 但浏览器仍然拦截，并提示：
  - `Access-Control-Allow-Origin` 不能是 `*`
  - 因为当前请求用了 `credentials: include`

为什么会这样：

- 前端请求插件里统一开启了 `credentials: 'include'`
- 这意味着浏览器会把它当成“带凭证的跨域请求”
- 带凭证时，后端响应头里的 `Access-Control-Allow-Origin` 就不能再是通配符 `*`
- 必须返回“明确的来源地址”

也就是说：

- `app.enableCors()` 的默认宽松写法
- 在简单接口联调时可能没问题
- 但一旦前端带上 cookie / 凭证模式，就必须改成明确 origin

最终做法：

- 在 `apps/ww-server/src/main.ts` 中把 CORS 改成显式配置
- 开启 `credentials: true`
- 只放行当前开发中会用到的前端来源
  - `http://localhost:*`
  - `http://127.0.0.1:*`
  - `http://10.10.70.216:*`
- 同时支持通过 `CORS_ORIGINS` 环境变量补充允许来源

经验：

- 当前端请求里用了 `credentials: include`
- 后端就不要再返回 `Access-Control-Allow-Origin: *`
- 这不是 NestJS 特有问题，而是浏览器的 CORS 规则

如果后面再遇到“接口地址对了，但浏览器还是拦”的情况，优先检查：

1. 前端是否带了 `credentials`
2. 后端 CORS 是否返回了明确 origin
3. 前后端实际访问域名 / 端口是否和 allowlist 一致

---

### 6. `0.0.0.0` 适合服务监听，不适合浏览器直接请求

这次继续联调时，又碰到了另一个很常见的问题：

现象：

- 后端明明已经监听在 `0.0.0.0:6789`
- 浏览器里却报：
  - `POST http://0.0.0.0:6789/api/user/login net::ERR_INVALID_HTTP_RESPONSE`

为什么会这样：

- `0.0.0.0` 的含义是“监听所有网卡地址”
- 它适合服务端 `listen`
- 但它不是浏览器里稳定可访问的目标地址
...
- 服务端可以写 `host = 0.0.0.0`
- 但前端请求地址应该写成真正能访问的主机名
  - 例如 `127.0.0.1`
  - 或者当前电脑的局域网 IP

最终做法：

- 前端默认 `apiBase` 改成更通用的 `http://127.0.0.1:6789/api`
- 新增 `apps/web-nuxt/.env.example`
- 新增 `apps/ww-server/.env.example`
- 前端增加一个运行时兜底：
  - 如果环境变量误写成 `http://0.0.0.0:6789/api`
  - 浏览器端会自动替换成当前页面的 hostname
...
- `0.0.0.0` 是“监听地址”
- `127.0.0.1`、`localhost`、局域网 IP 才是“访问地址”
...
- 学习项目里，功能优先没有问题
- 但如果页面层次太挤，会反过来影响你理解组件关系和信息结构

所以这一轮主要只做：

- 登录页左右栏留白优化
- 登录表单 tabs 和按钮节奏优化
- 面试入口 hero 与卡片区的上下间距优化
- 业务卡片统一改成更稳定的卡片容器写法

先把基础观感拉齐，再留到后面做更完整的 UI 升级，会更符合当前学习节奏。
